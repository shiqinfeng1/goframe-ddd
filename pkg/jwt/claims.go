...
type userClaims struct {
	...
	authorizer.UserClaims           // 自定义的用户信息声明
	jwt.RegisteredClaims            // 默认的信息声明
}
...
type deviceClaims struct {
	Kind                    claimKind `json:"kind"`
	authorizer.DeviceClaims           // 自定义的用户信息声明
	jwt.RegisteredClaims              // 默认的信息声明
}
...
func EncodeUserClaims(claims authorizer.UserClaims, privateKey *rsa.PrivateKey) (string, error) {
	...
	jwtClaims := userClaims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			...
			ExpiresAt: jwt.NewNumericDate(now.Add(time.Hour * 72)), // 默认有效期72小时
		},
	}
