...
func NewErrPublicKeyDataInvalid(value []byte, next error) error {
	...
	// the PublicKey field when it validate the public key data. When validating other fields, the validation
	...
	// value agreement.
	//
	// For now, there are a test to check if the PublicKey has the "Data" field.
...
func NewErrDeviceNotFound(id string, next error) error {
...
func NewErrSessionNotFound(id string, next error) error {
...
// NewErrNamespaceMemberFillData returns an error to be used when the conversion of Member, with only the ID and
...
func NewErrDeviceSetOnline(id string, err error) error {
