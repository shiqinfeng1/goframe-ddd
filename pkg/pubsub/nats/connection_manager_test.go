...
func TestNewConnectionManager(t *testing.T) {
	...
	natsConnector := &MockConnector{}
	jsCreator := &MockJetStreamCreator{}

	cm := newConnectionManager(cfg, natsConnector, jsCreator)
...
func TestConnectionManager_Connect(t *testing.T) {
	...
	mockConn := NewMockConnIntf(ctrl)
	mockJS := NewMockJetStream(ctrl)
	mockNATSConnector := NewMockConnector(ctrl)
...
func TestConnectionManager_Publish(t *testing.T) {
	...
	err := cm.Publish(ctx, subject, message)
	require.NoError(t, err)
}
...
func TestConnectionManager_validateJetStream(t *testing.T) {
	...
	err := cm.validateJetStream(t.Context(), "test.subject")
	require.NoError(t, err)
	...
	err = cm.validateJetStream(t.Context(), "")
	assert.Equal(t, errJetStreamNotConfigured, err)
}
...
func TestConnectionManager_Health(t *testing.T) {
	...
	assert.Equal(t, "nats://localhost:4222", h.Details["server"])
