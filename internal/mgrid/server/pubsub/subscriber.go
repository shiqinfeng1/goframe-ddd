...
type ControllerV1 struct {
	...
	ncfact        nats.ConnFactory // 连接到服务的连接管理器
	natsClient    *nats.Client
	app           application.Service
}

func NewV1(logger server.Logger, app application.Service, natsAddr string, ncfact nats.ConnFactory) *ControllerV1 {
	c := &ControllerV1{
		logger:        logger,
		subscriptions: make(map[string]nats.SubscribeFunc),
		consumes:      make(map[string]nats.ConsumeFunc),
		group:         errgroup.Group{},
		natsClient:    nats.New(logger, natsAddr),
		app:           app,
		ncfact:        ncfact,
	}
...
// 注册topic的处理函数
func (c *ControllerV1) attachSubscribeHandler(ctx context.Context, topic string, handler nats.SubscribeFunc) {
...
func (c *ControllerV1) attachConsumeHandler(ctx context.Context, topic string, handler nats.ConsumeFunc) {
...
func (c *ControllerV1) Run(ctx context.Context) error {
	...
	// 连接到nats服务端，用于接收消息
	connSub, err := c.ncfact.New(ctx, "mgrid sub client")
	if err != nil {
		return err
	}
	...
	// 连接到nats服务端，用于消费流
	connConsume, err := c.ncfact.New(ctx, "mgrid consume client")
	if err != nil {
		return err
	}
