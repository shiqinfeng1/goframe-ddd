...
func Images(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, dockerctl.HandlerResp{Code: -1, Message: fmt.Sprintf("获取镜像列表出错: %v", err)})
		return
	}
	...
	c.JSON(http.StatusOK, dockerctl.HandlerResp{Code: 0, Message: "查询镜像列表成功", Data: images})
}
